module Pavo
  class Renderer
    ...
    def self.content_partial_for(page, section)
      ...
      partial_path = "#{Pavo.config.app_root.to_s}#{Pavo.config.views_path}/#{page.underscore}/_#{section.underscore}.html.erb"
      if File.file?(partial_path)
        return partial_path
      else
        return "/Pavo/styleguide/_content.html.erb"
      end
    end
    ...
    def self.render(page, section)
      ...
      Pavo.config.paths.each do |path|
        @extensions.each do |extension|
          file_path = "#{Pavo.config.app_root.to_s}#{path}/#{page.underscore}/#{section.underscore}#{extension}"
